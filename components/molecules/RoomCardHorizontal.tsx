  ...
  return (
    <Link href={`/room/${room.id}`} className="no-underline block">
      <div className={cards.roomCardHorizontal}>
        ...
        <div className={`flex-1 ${spacing.card.paddingLarge} flex flex-col justify-between`}>
          {/* Tipo y precio */}
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
              <div className={`bg-[#0a174e] text-white rounded-lg px-4 py-2 font-bold ${typography.cardTitle} tracking-wide inline-block w-fit`}>
                {room.tipo}
              </div>
              <div className="text-[#0a174e] font-bold text-xl sm:text-2xl">
                {formatPrice(room.precio_noche)} <span className="text-base sm:text-lg font-normal text-gray-600">/ noche</span>
              </div>
            </div>

            {/* Descripción */}
            <p className={`${typography.body} text-gray-700 leading-relaxed mb-4 line-clamp-3 sm:line-clamp-4`}>
              {description}
            </p>

            {/* Características principales */}
            {mainFeatures.length > 0 && (
              <div className={`flex flex-wrap ${spacing.gap.small} mb-4`}>
